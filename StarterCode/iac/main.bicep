...
resource storageAccount 'Microsoft.Storage/storageAccounts@2023-01-01' = {
  ...
  kind: 'StorageV2'
  properties: { 
    allowBlobPublicAccess: true //HOX <--Piti lisätä ?
  }
}
...
resource appServicePlan 'Microsoft.Web/serverfarms@2022-09-01' = {
  ...
  location: location
  sku: {
    name: 'F1' //HOX <--Piti vaihtaa B1 -> F1
    tier: 'Free' //HOX <--Piti vaihtaa Basic -> Free
  }
  kind: 'linux'
  properties: {
    reserved: true
...
resource storageBlobRole 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(storageAccount.id, webApp.id, 'StorageBlobDataContributor') //HOX <- webApp.id eikä webApp.identity.principalId
...
resource keyVaultSecretsRole 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(keyVault.id, webApp.id, 'KeyVaultSecretsUser') //HOX <- webApp.id eikä webApp.identity.principalId
  scope: keyVault
  properties: {
